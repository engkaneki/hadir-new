...
class OditolakController extends Controller
{
    public function index()
    {
        $user = Auth::user();
        $profil_op = ProfilModel::where('kode_user', $user->kode_user)->first();
        $berkas = PengajuanKK::where('status', 'ditolak')->paginate(10);
    ...
    public function lahir()
    {
        $user = Auth::user();
        $profil_op = ProfilModel::where('kode_user', $user->kode_user)->first();
        $berkas = PengajuanAktaLahir::where('status', 'ditolak')->paginate(10);
        $usernamesPetugas = $berkas->pluck('petugas')->unique();
        $petugas = User::whereIn('username', $usernamesPetugas)->get()->keyBy('username');
        $berkas->transform(function ($item) use ($petugas) {
            $item->tgl_pengajuan = Carbon::parse($item->tgl_pengajuan)->translatedFormat('d F Y');
    ...
    public function kematian()
    {
        $user = Auth::user();
        $profil_op = ProfilModel::where('kode_user', $user->kode_user)->first();
        $berkas = PengajuanKematian::where('status', 'ditolak')->paginate(10);
        $usernamesPetugas = $berkas->pluck('petugas')->unique();
        $petugas = User::whereIn('username', $usernamesPetugas)->get()->keyBy('username');
        $berkas->transform(function ($item) use ($petugas) {
            $item->tgl_pengajuan = Carbon::parse($item->tgl_pengajuan)->translatedFormat('d F Y');
    ...
    public function pindah()
    {
        $user = Auth::user();
        $profil_op = ProfilModel::where('kode_user', $user->kode_user)->first();
        $berkas = PengajuanSuratPindah::where('status', 'ditolak')->paginate(10);
        $usernamesPetugas = $berkas->pluck('petugas')->unique();
        $petugas = User::whereIn('username', $usernamesPetugas)->get()->keyBy('username');
        $berkas->transform(function ($item) use ($petugas) {
            $item->tgl_pengajuan = Carbon::parse($item->tgl_pengajuan)->translatedFormat('d F Y');
